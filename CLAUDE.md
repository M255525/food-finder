...
- `mrt-stations-data.js` — 台北捷運＋新北捷運（環狀線／安坑輕軌／淡海輕軌／三鶯線）共 149 站，`window.MRT_STATIONS = [...]`
...
捷運站選擇是**兩層 `<select>`**（`#mrtLineSelect` → `#mrtStationSelect`），不是單一個 149 站平鋪的下拉/datalist——使用者回饋單層清單太混亂。`MRT_LINE_ORDER` 常數固定路線顯示順序（板南線／淡水信義線／松山新店線／中和新蘆線／文湖線／環狀線／安坑輕軌／淡海輕軌／三鶯線，依台北捷運官方慣用順序＋新北捷運線放最後，不是字母序），選路線後才動態用 `s.lines.indexOf(line)` 篩出該路線站別填進第二個 select（預設 `disabled`，避免使用者跳過選路線直接亂選）。轉乘站（例如「市政府」同時屬於板南線／文湖線的鄰站中山國中一帶，或「中山」同時在淡水信義線與松山新店線，或「中正紀念堂」同時在淡水信義線與松山新店線，或「頂埔」同時在板南線與三鶯線）**會在多條路線的清單裡各自出現一次**，這是刻意的、符合真實轉乘站邏輯，不是資料重複的 bug。**這裡的關鍵是同一個站要用「單一物件、`lines` 陣列放多個路線名稱」表示**（不是為每條路線各建一個重複物件），`initMrtOptions()` 才會靠 `s.lines.indexOf(line)` 讓它同時出現在多個路線的第二層清單裡；`MRT_LINE_ORDER` 也必須同步收錄該路線名稱，否則即使 `mrt-stations-data.js` 裡有資料，`orderedLines = MRT_LINE_ORDER.filter(...)` 這行也會把整條路線從第一層下拉選單濾掉（2026-08-16 新增三鶯線時踩過這個坑：先加了資料才發現路線選單沒同步更新）。

三鶯線（新北捷運營運，12站，2026-06-30通車）11個新站（頂埔為既有站，只是加掛三鶯線）的座標是依新北大眾捷運公司公告站址人工估算，**不是** Overpass 查詢結果，精度低於其他站；且中正紀念堂原本漏標松山新店線（只有淡水信義線）也已一併修正——這兩處都是 2026-08-16 使用者實測回報「有些路線/站別沒收錄」才發現的，`mrt-stations-data.js` 的資料在通車或轉乘資訊變動後不會自動更新，之後若又有新線通車或發現其他轉乘站漏標，要重複這套流程：先查證官方站名/站碼/轉乘關係，再同時改 `mrt-stations-data.js`（含新增/修正站資料）＋`index.html` 的 `MRT_LINE_ORDER`＋`README.md`／`manual.html`／本檔案裡出現的站數與路線清單，四處要一起改，漏改任何一處都會讓文件與實際行為對不上。
